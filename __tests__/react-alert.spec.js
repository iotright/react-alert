...
describe('react-alert', () => {
  ...
  describe('react-alert with one Provider and custom options', () => {
    ...
    it('should accept a containerStyle option', () => {
      ...
      const providerElement = getByTestId('provider')
      expect(providerElement).toHaveStyle('z-index: 50; border: 1px solid red;')
    })
    ...
    it('should render an alert in position passed to alert directly', () => {
      ...
      Child = () => {
        ...
        return (
          <button
            onClick={() => alert.show('Message', { position: alertPosition })}
          >
            Show Alert
          </button>
        )
    ...
    it('should close an alert automatic after the given alert timeout', async () => {
      const ChildWithConfig = () => {
        ...
        return (
          <button onClick={() => alert.show('Message', { timeout: 3000 })}>
            Show Alert
          </button>
        )
      }
      ...
      Child = () => {
        ...
        return (
          <div>
            <button
              onClick={() =>
                alert.show('Message', { position: positions.BOTTOM_LEFT })
              }
            >
              Bottom left
            </button>
            <button
              onClick={() =>
                alert.show('Message', { position: positions.BOTTOM_RIGHT })
              }
            >
              Bottom right
            </button>
